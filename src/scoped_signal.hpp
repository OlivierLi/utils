 ...
 public:
  ...
  SignalTimer(int sig, std::chrono::nanoseconds interval): 
    ...
    if (timer_create(CLOCK_MONOTONIC, &sev_, &timer_id_) == -1){
      std::cerr << "Timer creation failed. Aborting" << std::endl;
      std::abort();
    }
  ...
  void Start() {
    if(state_ == State::Uninitialized){
      std::cerr << "Timer stopping before starting. Aborting" << std::endl;
      std::abort();
    }
    ...
    if (timer_settime(timer_id_, 0, &timer_spec_, nullptr) == -1){
      std::cerr << "Timer setting failed. Aborting" << std::endl;
      std::abort();
    }
  ...
  void Stop() {
    ...
    if (timer_delete(timer_id_) == -1){
      std::cerr << "Timer deleting failed. Aborting" << std::endl;
      std::abort();
    }
