...
class has_find {
    private:
        ...
        template<typename C> static auto test(void*) -> decltype(std::declval<C>().find({}), std::true_type{});

        // This is the fallback function, if the decltype does not work at the previous step
    ...
    public:
        ...
        static constexpr bool value =  std::is_same<return_type, std::true_type>::value;
};

// This overload is made invalid if the find function is not present
template <typename K, typename C, std::enable_if_t<has_find<C>::value, std::nullptr_t> = nullptr>
...
template <typename K, typename C, std::enable_if_t<!has_find<C>::value, std::nullptr_t> = nullptr>
bool is_in(K key, const C& container){
    return std::find(std::begin(container), std::end(container), key) != container.end();
}
